...
if [ -f "./clash.yaml" ]; then
    ...
    in_proxy == 1 && /^[ ]{2,}- name:/ { 
        ...
        if (in_current_proxy == 1) {
            # 检查是否需要保存前一个节点
            if (remove_current == 0 && (current_ip == "" || ! (current_ip in valid_ips))) {
                if (current_ip != "") {
                    valid_ips[current_ip] = 1
                }
        ...
        # 检查是否包含无效的cipher或password
        if (index($0, "cipher: \"\"") > 0 || index($0, "password: \"\"") > 0) {
            remove_current = 1
            if (current_name != "") {
                deleted_nodes[current_name] = 1
            }
        }
    ...
    # 处理proxy-groups中的proxies列表
    in_proxy_groups == 1 && /proxies: *\[.*\]/ {
        # 处理内联proxies列表
        line = $0
        gsub(/^[ ]*proxies: *\[/, "", line)
        gsub(/\][ ]*$/, "", line)
        ...
        for (i = 1; i <= n; i++) {
            gsub(/^[ "]*|[ "]*$/, "", items[i])
            if (items[i] != "" && ! (items[i] in deleted_nodes)) {
                if (first == 0) {
                    printf ","
                }
                printf "\"%s\"", items[i]
    ...
    in_proxy_groups == 1 && in_proxies_list == 1 && /^[ ]+- / {
        node_name = $0
        gsub(/^[ ]+- "?|"?$/, "", node_name)
    ...
    # 默认输出
    {
        if (in_proxy == 0 && in_proxy_groups == 0) {
            print $0
        } else if (in_proxy_groups == 1 && !in_proxies_list) {
            print $0
        }
        # 其他情况已在上面处理
    }
